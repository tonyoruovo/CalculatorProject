 ...
 */
/**
 * Static methods for Excess-<span style="font-style:italic">k</span> arithmetic on a bounded {@code BigInteger}. Bounded
 * integers are {@code BigInteger} values that behave exactly like their native counterparts therefore can be used to simulate
 * values such as {@code byte} (bound to 8-bits), {@code short} (bound to 16-bits), {@code int} (bound to 32-bits), {@code long}
 * (bound to 64-bits) and also create dyadic bounds such as 128, 256 and so on. <p>Unless otherwise stated, all values to be
 * operated on are in decimal format and must be non-null. The results are all in Excess-<span
 * style="font-style:italic">k</span> format.
 * 
...
public class Ex {
	...
	/*
	 * Date: 28 Nov 2023 -----------------------------------------------------------
	 * Time created: 08:07:59 ---------------------------------------------------
	 */
	/**
	 * Constructor for creating a {@code mathaid.calculator.base.value.Ex} object. It is {@code private} because this class is not
	 * to be instantiated.
	 */
	private Ex() {
	}
	 ...
	 */
	/**
	 * Creates a {@code Comparator} that can do comparisons relative to Excess-<span style="font-style:italic">k</span> format.
	 * <p>All operands of the comparison are assumed to be in decimal representation i.e ordinary integers. <p>The
	 * {@code Comparator} itself will covert both operands from decimal to 2's complement format where the actual comparison will
	 * then be made and the results returned.
	 * 
	 * @param bound the bit length for both of the operands.
	 * 
	 * @return A {@code Comparator} specifically integers in the given bound for Excess-<span style="font-style:italic">k</span>
	 *         formats.
	 */
	 ...
	 */
	/**
	 * Converts the first element of {@code r} (which is assumed to be in decimal format) to Excess-<span
	 * style="font-style:italic">k</span> format and stores the result at the same index.
	 * 
	 ...
	 */
	/**
	 * Performs an Excess-<span style="font-style:italic">k</span> left shift on the first element of {@code r}, storing the result
	 * in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is always a
	 * logical left shift.
	 * 
	 ...
	 */
	/**
	 * Performs an Excess-<span style="font-style:italic">k</span> (signed if specified) right shift on the first element of
	 * {@code r}, storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This
	 * bitwise operation is a logical right shift if {@code a == false} else it is a arithmetic shift.
	 * 
	 * @param s the number of bit (i.e the bit distance) to which the value will be shifted.
	 * @param b the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256, 512
	 *          and so on. This is the bit-length of the value to be converted.
	...
	private static void sr(int s, int b, boolean a, BigInteger[] r) {
		...
		if (isNeg && a && r[0].bitLength() < b)
			r[0] = clearMSB(getTrailingZeros(b - 1).or(r[0]), 1);
	}
	 ...
	 */
	/**
	 * Performs an Excess-<span style="font-style:italic">k</span> circular/rotating left shift on the first element of {@code r},
	 * storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise
	 * operation is always a logical (unsigned) circular left shift.
	 * 
	 * @param s the number of bit (i.e the bit distance) to which the value will be shifted.
	 * @param b the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256, 512
	 *          and so on. This is the bit-length of the value to be converted.
	 ...
	 */
	/**
	 * Performs an Excess-<span style="font-style:italic">k</span> circular/rotating right shift on the first element of {@code r},
	 * storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise
	 * operation is always a logical (unsigned) circular right shift.
	 * 
	 * @param s the number of bit (i.e the bit distance) to which the value will be shifted.
	 * @param b the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256, 512
	 *          and so on. This is the bit-length of the value to be converted.
	 ...
	 * <code>&lt;&lt;</code> operator on the first element of the given {@code r} which also emulates a Excess-<span
	 * style="font-style:italic">k</span> integer of the same bit length as {@code bound}. When the value to be converted is signed
	 * (negative), the operation is performed on it's Excess-<span style="font-style:italic">k</span> equivalent. This means that if
	 * shifting will cause the argument to have a bigger bit length than the given bound, the excess bits are allowed to "fall off".
	 * The last bit among those that "fell off" becomes the carry bit and it is store at the second element of {@code r}, this index
	 * then becomes the carry/overflow bit. <p> This method is created so as allow for simulation of java's <code>&lt;&lt;</code>
	 * operator when porting mathematical libraries from primitive types that use Excess-<span style="font-style:italic">k</span>
	 * bit operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift argument is
	 * negative.
	 * 
	 ...
	 *              converted to the Excess-<span style="font-style:italic">k</span> equivalent with respect to bound. This means
	 *              that if this method is called with a {@code BigInteger} equivalent to 400 and bound equal to 8, then the
	 *              {@code BigInteger} will be truncated before the operation will commence. The second index contains the
	 *              overflow/carry bit.
	 * 
	 ...
	 * which also emulates a Excess-<span style="font-style:italic">k</span> integer of the same bit length as {@code bound}. When
	 * the value to be converted is signed (negative), the operation is performed on it's Excess-<span
	 * style="font-style:italic">k</span> equivalent. <p> This method is created so as allow for simulation of java's
	 * <code>&gt;&gt;&gt;</code> (or the <code>&gt;&gt;</code> if the {@code arithmetic} flag is set to {@code true}) bitwise
	 * operator when porting mathematical libraries from primitive types that use Excess-<span style="font-style:italic">k</span>
	 * bit operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift argument is
	 * negative.
	 * 
	 ...
	 *                   lossless computation was done).<p>Note that this value will be converted to the Excess-<span
	 *                   style="font-style:italic">k</span> equivalent with respect to bound. This means that if this method is
	 *                   called with a {@code BigInteger} equivalent to 400 and bound equal to 8, then the {@code BigInteger} will
	 *                   be truncated before the operation will commence.
	 * 
	 * @throws ArithmeticException if bound is less than 4 or if a catastrophic overflow occurred such that the loss of significant
	 *                             bits was too large to ignore and would result in an very wrong result. A 'catastrophic overflow'
	 *                             in this context is when {@code bound} is too large so that a multiplication by 2 would overflow
	 ...
	 */
	/**
	 * Performs a Excess-<span style="font-style:italic">k</span> circular/rotating left shift on the first element of {@code r},
	 * storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This emulates a
	 * circular left shift (left rotation) on the given array {@code r} (which contains the value to shifted and the carry bit)
	 * which also emulates an unsigned integer of the same bit length as {@code bound} and returns the {@code BigInteger} and the
	 * new carry bit. When {@code n} is signed (negative), the operation is performed on it's unsigned equivalent, and then
	 * converted to the signed equivalent, preserving the sign only if the sign bit was not moved or. <p> This method is created so
	 * as allow for simulation of the circular shift operations when porting mathematical libraries from primitive types that use
	 * Excess-<span style="font-style:italic">k</span> bit operations. <p> Note that as with java's shift operators, this method
	 * becomes undefined if the shift argument is negative
	 * 
	 ...
	 *              value will be converted to the Excess-<span style="font-style:italic">k</span> equivalent with respect to bound.
	 *              This means that if this method is called with a {@code BigInteger} equivalent to 400 and bound equal to 8, then
	 *              the {@code BigInteger} will be truncated before the operation will commence.
	 ...
	 */
	/**
	 * Performs a Excess-<span style="font-style:italic">k</span> circular/rotating right shift on the first element of {@code r},
	 * storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This emulates a
	 * circular right shift (right rotation) on the given array {@code r} (which contains the value to shifted and the carry bit)
	 * which also emulates an unsigned integer of the same bit length as {@code bound} and returns the {@code BigInteger} and the
	 * new carry bit. When {@code n} is signed (negative), the operation is performed on it's unsigned equivalent, and then
	 * converted to the signed equivalent, preserving the sign only if the sign bit was not moved or. <p> This method is created so
	 * as allow for simulation of the circular shift operations when porting mathematical libraries from primitive types that use
	 ...
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be computed. This assumes that the first index is in Excess-<span
	 *              style="font-style:italic">k</span> format. When this method returns, the first index contains the computed value
	 *              and the second is either {@code 1} or {@code 0}, however, it holds no consequence here.
	 * 
