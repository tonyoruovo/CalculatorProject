 ...
 */
/**
 * Static methods for Negabinary arithmetic on a bounded {@code BigInteger}. Bounded integers are {@code BigInteger} values that
 * behave exactly like their native counterparts therefore can be used to simulate values such as {@code byte} (bound to
 ...
 * must be non-null. The results are all in negabinary format.
 * 
...
public class NB {
	...
	/*
	 * Date: 28 Nov 2023 -----------------------------------------------------------
	 * Time created: 08:06:30 ---------------------------------------------------
	 */
	/**
	 * Constructor for creating a {@code mathaid.calculator.base.value.NB} object.
	 ...
	 */
	/**
	 * Creates a {@code Comparator} that can do comparisons relative to Negabinary format.
	 ...
	 * 
	 * @return A {@code Comparator} specifically integers in the given bound for Negabinary
	 *         formats.
	 */
	 ...
	 */
	/**
	 * Converts the first element of {@code r} (which is assumed to be in decimal format) to Negabinary format and stores the result at the same index.
	 * 
	 ...
	 */
	/**
	 * Performs a Negabinary left shift on the first element of {@code r}, storing the result
	 ...
	 */
	/**
	 * Performs a Negabinary (signed if specified) right shift on the first element of
	 ...
	 *                             when {@code b} is too large so that a multiplication by 2 would overflow it's 32 bits allocated.
	 */
	private static void sr(int s, int b, boolean a, BigInteger[] r) {//unsigned
		if (Math.abs(s) > b) {
			sr(s % b, b, a, r);
			return;
		}
	 ...
	 */
	/**
	 * Performs a Negabinary circular/rotating left shift on the first element of {@code r},
	 ...
	 */
	/**
	 * Performs a Negabinary circular/rotating right shift on the first element of {@code r},
	 * storing the result in the same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise
	 ...
	 * <code>&lt;&lt;</code> operator on the first element of the given {@code r} which also emulates a negabinary integer of
	 * the same bit length as {@code bound}. When the value to be converted is signed (negative), the operation is performed on it's
	 * negabinary equivalent. This means that if shifting will cause the argument to have a bigger bit length than the given
	 ...
	 * use negabinary bit operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift
	 ...
	 *              converted to the negabinary equivalent with respect to bound. This means that if this method is called with
	 ...
	 * which also emulates a negabinary integer of the same bit length as {@code bound}. When the value to be converted is
	 * signed (negative), the operation is performed on it's negabinary equivalent. <p> This method is created so as allow for
	 * simulation of java's <code>&gt;&gt;&gt;</code> (or the <code>&gt;&gt;</code> if the {@code arithmetic} flag is set to
	 * {@code true}) bitwise operator when porting mathematical libraries from primitive types that use negabinary bit
	 ...
	 *                   lossless computation was done).<p>Note that this value will be converted to the negabinary equivalent
	 ...
	 */
	/**
	 * Performs a negabinary circular/rotating left shift on the first element of {@code r}, storing the result in the same
	 ...
	 * operations when porting mathematical libraries from primitive types that use negabinary bit operations. <p> Note that as
	 ...
	 *              value will be converted to the negabinary equivalent with respect to bound. This means that if this method
	 ...
	 */
	/**
	 * Performs a negabinary circular/rotating right shift on the first element of {@code r}, storing the result in the same
	 ...
	 * of the circular shift operations when porting mathematical libraries from primitive types that use negabinary bit
	 * operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift argument is negative
	 * 
	 * @param r     a 2-length array of a {@code BigInteger} values (representing the value to be shifted) and a {@code BigInteger}
	 *              (representing the carry bit. Only the least significant bit is needed. Although this value is ignored if the
	 ...
	 *                             it's 32 bits allocated.
	 */
	public static void circularShiftRight(int shift, int bound, boolean carry, BigInteger[] r) throws ArithmeticException {
		if (bound < 4)
			throw new ArithmeticException("bound is less than 4");
	 ...
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be computed. This assumes that the first index is in Negabinary format. When this method returns, the first index contains the computed value
