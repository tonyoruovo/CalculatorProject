 ...
 */
/**
 * Static methods for sign and magnitude arithmetic on a bounded {@code BigInteger}. Bounded integers are {@code BigInteger}
 ...
 * create dyadic bounds such as 128, 256 and so on. <p>Unless otherwise stated, all values to be operated on are in decimal
 * format and must be non-null. The results are all in signed and magnitude format.
 * 
...
public class SMR {
	...
	/*
	 * Date: 28 Nov 2023 -----------------------------------------------------------
	 * Time created: 08:09:16 ---------------------------------------------------
	 */
	/**
	 * Constructor for creating a {@code mathaid.calculator.base.value.SMR} object.
	 * It is {@code private} because this class is not to be instantiated.
	 */
	private SMR() {}
	 ...
	 */
	/**
	 * Creates a {@code Comparator} that can do comparisons relative to Signed and magnitude format. <p>All operands of the
	 * comparison are assumed to be in decimal representation i.e ordinary integers. <p>The {@code Comparator} itself will covert
	 * both operands from decimal to 2's complement format where the actual comparison will then be made and the results returned.
	 * 
	 * @param bound the bit length for both of the operands.
	 * 
	 * @return A {@code Comparator} specifically integers in the given bound for Signed and magnitude formats.
	 */
	 ...
	 */
	/**
	 * Converts the first element of {@code r} (which is assumed to be in decimal format) to Signed and magnitude format and stores
	 * the result at the same index.
	 * 
	 ...
	 */
	public static void fromDecimal(int bound, BigInteger[] r) {
		if (bound < 4)
		...
		if (r[0].signum() < 0) {
			...
			if (r[0].bitLength() < bound) {
				r[0] = getTrailingZeros(bound - 1).or(r[0]);
				return;
			}

			fromDecimal(bound, r);
			return;
		} else if (r[0].bitLength() > bound) {
	 ...
	 */
	/**
	 * Performs the Signed and magnitude 'not' on the first element of the array {@code r} and replaces that element with the result
	 * of this computation. <p>The element whose 'not' is to be calculated is assumed to be in decimal formatted integer.
	 * 
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be transformed. When this method returns, the first index contains the
	 ...
	 */
	/**
	 * Performs the Signed and magnitude 'negate' on the first element of the array {@code r} and replaces that element with the
	 * result of this computation. <p>The element whose 'negate' is to be calculated is assumed to be in decimal formatted integer.
	 * 
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be transformed. When this method returns, the first index contains the
	 ...
	 * @throws ArithmeticException if {@code bound < 4}.
	 */
	public static void negate(int bound, BigInteger[] r) throws ArithmeticException {
		r[0] = r[0].negate();
		fromDecimal(bound, r);
	}
	 ...
	 */
	/**
	 * Performs a Signed and magnitude left shift on the first element of {@code r}, storing the result in the same index of
	 * {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is always a logical left shift.
	 * 
	 ...
	 */
	/**
	 * Performs a Signed and magnitude (signed if specified) right shift on the first element of {@code r}, storing the result in
	 * the same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is a logical right
	 * shift if {@code a == false} else it is a arithmetic shift.
	 * 
	 * @param s the number of bit (i.e the bit distance) to which the value will be shifted.
	 * @param b the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256, 512
	 *          and so on. This is the bit-length of the value to be converted.
	 ...
	 */
	/**
	 * Performs a Signed and magnitude circular/rotating left shift on the first element of {@code r}, storing the result in the
	 * same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is always a logical
	 ...
	 */
	/**
	 * Performs a Signed and magnitude circular/rotating right shift on the first element of {@code r}, storing the result in the
	 * same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is always a logical
	 ...
	 * <code>&lt;&lt;</code> operator on the first element of the given {@code r} which also emulates a signed and magnitude integer of
	 * the same bit length as {@code bound}. When the value to be converted is signed (negative), the operation is performed on it's
	 * signed and magnitude equivalent. This means that if shifting will cause the argument to have a bigger bit length than the given
	 ...
	 * use signed and magnitude bit operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift
	 ...
	 *              converted to the signed and magnitude equivalent with respect to bound. This means that if this method is called with
	 ...
	 * which also emulates a signed and magnitude integer of the same bit length as {@code bound}. When the value to be converted is
	 * signed (negative), the operation is performed on it's signed and magnitude equivalent. <p> This method is created so as allow for
	 * simulation of java's <code>&gt;&gt;&gt;</code> (or the <code>&gt;&gt;</code> if the {@code arithmetic} flag is set to
	 * {@code true}) bitwise operator when porting mathematical libraries from primitive types that use signed and magnitude bit
	 ...
	 *                   lossless computation was done).<p>Note that this value will be converted to the signed and magnitude equivalent
	 ...
	 */
	/**
	 * Performs a signed and magnitude circular/rotating left shift on the first element of {@code r}, storing the result in the same
	 ...
	 * operations when porting mathematical libraries from primitive types that use signed and magnitude bit operations. <p> Note that as
	 ...
	 *              value will be converted to the signed and magnitude equivalent with respect to bound. This means that if this method
	 ...
	 */
	/**
	 * Performs a signed and magnitude circular/rotating right shift on the first element of {@code r}, storing the result in the same
	 ...
	 * of the circular shift operations when porting mathematical libraries from primitive types that use signed and magnitude bit
	 * operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift argument is negative
	 * 
	 * @param r     a 2-length array of a {@code BigInteger} values (representing the value to be shifted) and a {@code BigInteger}
	 *              (representing the carry bit. Only the least significant bit is needed. Although this value is ignored if the
	 ...
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be computed. This assumes that the first index is in Signed and magnitude format.
	 *              When this method returns, the first index contains the computed value and the second is either {@code 1} or
