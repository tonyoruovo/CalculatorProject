 ...
 */
/**
 * Static methods for Unsigned binary arithmetic on a bounded {@code BigInteger}. Bounded integers are {@code BigInteger} values
 ...
 * must be non-null. The results are all in unsigned binary format.
 * 
...
public class US {
	...
	/*
	 * Date: 28 Nov 2023 -----------------------------------------------------------
	 * Time created: 08:08:55 ---------------------------------------------------
	 */
	/**
	 * Constructor for creating a {@code mathaid.calculator.base.value.US} object. It is {@code private} because this class is not
	 * to be instantiated.
	 */
	private US() {
	}
	 ...
	 */
	/**
	 * Creates a {@code Comparator} that can do comparisons relative to Unsigned binary format. <p>All operands of the comparison
	 * are assumed to be in decimal representation i.e ordinary integers. <p>The {@code Comparator} itself will covert both operands
	 ...
	 * 
	 * @return A {@code Comparator} specifically integers in the given bound for Unsigned binary formats.
	 */
	 ...
	 */
	/**
	 * Converts the first element of {@code r} (which is assumed to be in decimal format) to Unsigned binary format and stores the
	 ...
	 */
	/**
	 * Performs a Unsigned binary left shift on the first element of {@code r}, storing the result in the same index of {@code r}
	 * and the overflow bit (if any) in the second index. <p>This bitwise operation is always a logical left shift.
	 * 
	 ...
	 */
	/**
	 * Performs a Unsigned binary (signed if specified) right shift on the first element of {@code r}, storing the result in the
	 * same index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is a logical right shift
	 * if {@code a == false} else it is a arithmetic shift.
	 * 
	 * @param s the number of bit (i.e the bit distance) to which the value will be shifted.
	 * @param b the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256, 512
	 *          and so on. This is the bit-length of the value to be converted.
	 ...
	 */
	/**
	 * Performs a Unsigned binary circular/rotating left shift on the first element of {@code r}, storing the result in the same
	 ...
	 */
	/**
	 * Performs a Unsigned binary circular/rotating right shift on the first element of {@code r}, storing the result in the same
	 * index of {@code r} and the overflow bit (if any) in the second index. <p>This bitwise operation is always a logical
	 ...
	 * <code>&lt;&lt;</code> operator on the first element of the given {@code r} which also emulates a unsigned binary integer of
	 * the same bit length as {@code bound}. When the value to be converted is signed (negative), the operation is performed on it's
	 * unsigned binary equivalent. This means that if shifting will cause the argument to have a bigger bit length than the given
	 ...
	 * use unsigned binary bit operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift
	 ...
	 *              converted to the unsigned binary equivalent with respect to bound. This means that if this method is called with
	 ...
	 * which also emulates a unsigned binary integer of the same bit length as {@code bound}. When the value to be converted is
	 * signed (negative), the operation is performed on it's unsigned binary equivalent. <p> This method is created so as allow for
	 * simulation of java's <code>&gt;&gt;&gt;</code> (or the <code>&gt;&gt;</code> if the {@code arithmetic} flag is set to
	 * {@code true}) bitwise operator when porting mathematical libraries from primitive types that use unsigned binary bit
	 ...
	 *                   lossless computation was done).<p>Note that this value will be converted to the unsigned binary equivalent
	 ...
	 * operations when porting mathematical libraries from primitive types that use unsigned binary bit operations. <p> Note that as
	 ...
	 *              value will be converted to the unsigned binary equivalent with respect to bound. This means that if this method
	 ...
	 * of the circular shift operations when porting mathematical libraries from primitive types that use unsigned binary bit
	 * operations. <p> Note that as with java's shift operators, this method becomes undefined if the shift argument is negative
	 * 
	 * @param r     a 2-length array of a {@code BigInteger} values (representing the value to be shifted) and a {@code BigInteger}
	 *              (representing the carry bit. Only the least significant bit is needed. Although this value is ignored if the
	 ...
	 * @param bound the number of bits (dyadic) to work with. This simulates computer bit registers such as 8, 16, 32, 64, 128, 256,
	 *              512 and so on. This is the bit-length of the value to be converted.
	 * @param r     the array holding the value to be computed. This assumes that the first index is in Unsigned binary format. When
